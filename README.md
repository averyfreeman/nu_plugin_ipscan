# `ipscan`: local-network discovery for Rust and Nushell

[![Build](https://github.com/averyfreeman/nu_plugin_ipscan/actions/workflows/ci.yml/badge.svg)](https://github.com/averyfreeman/nu_plugin_ipscan/actions)

Find hosts on an authorized local IPv4 network with a focused Rust ARP scanner.
The standalone `ipscan` CLI keeps the original text, JSON, YAML, and CSV modes;
the companion `nu_plugin_ipscan` exposes the JSON mode as native Nushell values.
...
Download the `ipscan` binary for Linux (Ubuntu, Fedora, Debian, ...). See the [releases page](https://github.com/averyfreeman/nu_plugin_ipscan/releases) for other binaries.

```bash
wget -O ipscan https://github.com/averyfreeman/nu_plugin_ipscan/releases/download/v0.15.1/ipscan-v0.15.1-x86_64-unknown-linux-musl && chmod +x ./ipscan
```
...
```bash
git clone https://github.com/averyfreeman/nu_plugin_ipscan.git
cd nu_plugin_ipscan
cargo build --release
# Binary will be available at ./target/release/ipscan
```
...
```bash
wget -O /usr/share/ipscan/ieee-oui.csv http://standards-oui.ieee.org/oui/oui.csv
```
...
Use a [custom OUI MAC file](http://standards-oui.ieee.org/oui/oui.csv), the default path will be set to `/usr/share/ipscan/ieee-oui.csv"`.
...
Feel free to suggest an improvement, report a bug, or ask something: https://github.com/averyfreeman/nu_plugin_ipscan/issues

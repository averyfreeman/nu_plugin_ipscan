...
use ipscan_lib::utils::select_default_interface;
...
pub struct InterfaceSummary {
    ...
    /// Configured addresses rendered as CIDR strings.
    pub ips: Vec<String>,
    /// Administrative up state.
    pub is_up: bool,
    /// Loopback marker.
    pub is_loopback: bool,
    /// Broadcast capability marker.
    pub is_broadcast: bool,
    /// Multicast capability marker.
    pub is_multicast: bool,
}

/// Export interface inventory as structured JSON for `nu_plugin_ipscan`.
pub fn export_interfaces_json(interfaces: &[NetworkInterface]) -> String {
    let rows = interfaces
        .iter()
        .map(|interface| InterfaceSummary {
            ...
            ips: interface.ips.iter().map(ToString::to_string).collect(),
        ...
        .collect::<Vec<_>>();
    serde_json::to_string(&rows).unwrap_or_else(|error| {
        eprintln!("Could not export interface JSON ({error})");
        process::exit(1);
    })
}

//! Clap grammar and conversion helpers for the `ipscan` executable.
...
use ipscan_lib::scan_options::{OutputFormat, ProfileType, ScanOptions, ScanTiming};
use ipscan_lib::time::parse_to_milliseconds;
...
const EXAMPLES_HELP: &str = "EXAMPLES:
    ...
    ipscan -i eth0 -n 10.37.3.1,10.37.4.55/24

    # Launch a scan on a specific interface with a custom IP and stealth profile
    ipscan -i eth0 --source-ip 192.168.0.42 --profile stealth
...
pub fn build_args() -> Command {
    Command::new("ipscan")
        .version(CLI_VERSION)
        .about("A minimalistic local-network discovery tool written in Rust")
        .arg(
            Arg::new("profile")
                .short('p')
                .long("profile")
        ...
        .arg(
            Arg::new("oui-file")
                .long("oui-file")
                .value_name("FILE_PATH")
                .default_value("/usr/share/ipscan/ieee-oui.csv")
...
pub fn build_scan_options_from_args(matches: &ArgMatches) -> Arc<ScanOptions> {
    ...
    let oui_file: String = match matches.get_one::<String>("oui-file") {
        ...
        None => "/usr/share/ipscan/ieee-oui.csv".to_string(),
    };

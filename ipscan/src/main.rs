//! Standalone `ipscan` command-line entry point.
//!
//! This binary owns argument parsing, capability checks, signal handling, and
//! presentation. The packet and response logic lives in `ipscan-lib`, while
//! the separate `nu_plugin_ipscan` binary exposes JSON results to Nushell.
...
use crate::cli_utils::{
    display_scan_results, export_interfaces_json, export_to_csv, export_to_json, export_to_yaml,
    has_network_capability, print_ascii_packet, show_interfaces,
};
use ipscan_lib::{
...
fn main() {
    ...
    if matches.get_flag("list") {
        if matches
            ...
            .is_some_and(|output| output == "json")
        {
            println!("{}", export_interfaces_json(&interfaces));
        } else {

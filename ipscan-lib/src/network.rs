...
/// Maximum receive wait between checks of the shared timeout flag, in ms.
...
pub struct ScanEstimation {
    ...
    pub interval_ms: u64,
    /// Estimated complete scan duration, in milliseconds.
    pub duration_ms: u128,
    /// Estimated bytes sent across all requests.
    pub request_size: u128,
    /// Estimated request bandwidth, in bits per second.
    pub bandwidth: u128,
}
...
pub struct ResponseSummary {
    ...
    /// Number of packets accepted as ARP frames.
    pub arp_count: usize,
    /// Duration of response collection, in milliseconds.
    pub duration_ms: u128,
}
...
pub struct TargetDetails {
    ...
    pub ipv4: Ipv4Addr,
    /// Hardware address reported by the responding host.
    pub mac: MacAddr,
    /// Optional reverse-DNS name.
    pub hostname: Option<String>,
    /// Optional vendor name from the OUI database.
...
impl NetworkIterator {
    /// Create an iterator over one or more IPv4 network ranges.

...
/// Look up MAC-address vendors in an IEEE OUI CSV database.
///
/// An OUI is the 24-bit prefix assigned to a hardware vendor. The reader is
/// optional so scans can continue when the database file is unavailable.
pub struct Vendor {
    reader: Option<Reader<File>>,
}

impl Vendor {
    /// Create a resolver for the database at `path`.
    ///
    /// An unavailable file leaves the resolver disabled instead of failing the
    /// network scan.
    ...
    /// Return whether the CSV database was opened successfully.
    pub fn has_vendor_db(&self) -> bool {
        self.reader.is_some()
    }

    /// Find the vendor associated with a MAC address, if present.
